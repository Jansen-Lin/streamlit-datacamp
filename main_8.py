...
@st.cache(suppress_st_warning=True) # allows app to stay performant even when loading data from the web, manipulating large datasets, or performing expensive computations
def get_fvalue(val):
...
if app_mode=='Home':
    ...
    st.write('App realised by : JX LIN')  
    st.image('src\loan_image.jpg')
    st.markdown('Dataset :')
...
# In Prediction page
    ...
    st.image('src\slider-short-3.jpg')
